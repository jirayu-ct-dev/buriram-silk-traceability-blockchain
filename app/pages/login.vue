...
const ACTORS: { role: DashboardRole, label: string, description: string, icon: Component, demoUsername: string }[] = [
  {
    role: 'WEAVER',
    label: 'ช่างทอ',
    description: 'ลงทะเบียนผ้าไหม แนบหลักฐาน และติดตามผลการตรวจ',
    icon: markRaw(Spool),
    demoUsername: 'weaver_demo',
  },
  {
...
const handleSubmit = async () => {
  ...
  try {
    const response = await $fetch<{ role: DashboardRole }>('/api/auth/login', {
      method: 'POST',
      body: form.value,
    })
    ...
    await navigateTo(homeFor(response.role))
  } catch (error: unknown) {
    // Fallback demo mode: ถ้า API ยังไม่พร้อม (ไม่มี server) ให้ใช้ demoUsername → role โดยตรง
    const demo = ACTORS.find(a => a.demoUsername === form.value.username.trim())
    if (demo && form.value.password === 'demo1234') {
      role.value = demo.role
      toast.success('เข้าสู่ระบบสำเร็จ (โหมดตัวอย่าง)')
      await navigateTo(homeFor(demo.role))
      isSubmitting.value = false
      return
    }
    const err = error as { data?: { message?: string } }
    const message = err.data?.message ?? 'ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง'
...
const handleDemoLogin = async (actor: typeof ACTORS[number]) => {
  form.value.username = actor.demoUsername
          ...
          <ul class="space-y-2">
            <li v-for="actor in ACTORS" :key="actor.role">
              <button type="button" class="flex w-full items-center gap-2 rounded-lg border border-neutral-200 px-3 py-2 text-left text-xs text-neutral-600 hover:bg-neutral-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary" @click="handleDemoLogin(actor)">
                ...
                <code class="flex-1 bg-neutral-100 px-2 py-0.5 rounded text-neutral-700 font-mono">{{ actor.demoUsername }}</code>
                <span class="text-neutral-400">/</span>
                <code class="bg-neutral-100 px-2 py-0.5 rounded text-neutral-700 font-mono">demo1234</code>
              </button>
            </li>
          </ul>
          <p class="mt-2 text-[11px] text-neutral-500">หรือกรอกด้วยตัวเองด้านบน (เมื่อ API พร้อมจะตรวจรหัสผ่านจริง)</p>
        </div>
